...
Roller::Roller(short const p_xOrgin, short const p_yOrgin, short const p_length, short const p_width, std::vector<std::string>& p_roller_options,
...
void Roller::set_state(const unsigned short p_state){
  m_state = p_state;
  //lv_ddroller_set_selected(m_roller, m_state);
  update_roller();
}
...
void Roller::draw_roller(Screen& p_screen){
  ...
  lv_obj_set_width(m_roller, m_width);

  // lv_ddRoller_set_style(m_Roller, LV_DDRoller_STYLE_BG, m_background_style);
  // lv_ddRoller_set_style(m_Roller, LV_DDRoller_STYLE_SEL, m_selected_style);

  //lv_ddRoller_set_selected(m_Roller, m_state);
}

void Roller::update_roller(){
  // m_state = lv_ddRoller_get_selected(m_Roller);
  //
  // if(m_conected_int){
